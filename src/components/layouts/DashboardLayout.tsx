...
const DashboardLayout = ({ children, headerTitle, headerSubtitle, headerAction }) => {
  ...
  const navigationItems = getNavigationItems(followers).map((item) =>
    location.pathname === item.href ? { ...item, active: true } : item
  );

  return (
    <div className="min-h-screen bg-gradient-to-br from-background to-gray-50/30 flex">
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? "w-72" : "w-20"} bg-white border-r shadow-lg transition-all duration-300 flex flex-col`}>
          ...
          {navigationItems.map((item) => (
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl mb-2 ${
                item.active ? "bg-gray-100" : "text-gray-600 hover:bg-gray-50"
              }`}
            >
